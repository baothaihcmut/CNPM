...
public class TransactionService {
    ...
    public TransactionResult createTransaction(CreateTransactionCommand transactionCommand) throws Exception {
        try {
            ...
            transaction.getTransactionPaperQuantities().values().stream().forEach((paperQuantity) -> {
                customer.subtractPaper(paperQuantity);
            });
    ...
    public PaginatedResult<AdminTransactionResult> findTransaction(FindTransactionQuery query) {
        ...
        return PaginatedResult.of(res.getData().stream()
                .map((tramsacton) -> this.transactionMapper.toAdminTransactionResult(tramsacton))
                .toList(),
    ...
    public PaginatedResult<AdminTransactionResult> searchTransaction(SearchTransactionQuery search) {
        String[] fields = { "name", "employee.user.firstName", "employee.user.lastName", "employee.employeeId",
                "customer.user.firstName", "customer.user.lastName", "customer.employeeId",
                "transactionId" };
        List<FilterParam<?>> filters = new ArrayList<>();
        ...
        PaginatedResult<Transaction> res = this.transactionRepositoy.search(List.of(statusCriteria), filters,
