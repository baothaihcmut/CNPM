...
public class JpaTransactionRepository implements TransactionRepositoy {
        ...
        @Override
        public PaginatedResult<Transaction> findByCriteria(List<FilterParam<?>> filters, List<SortParam> sorts,
                ...
                List<Transaction> transactions = this.entityManager.createQuery(cq).setFirstResult(offset)
                                .setMaxResults(pagniate.getSize()).getResultList()
                                .stream()
                                .map((transaction) -> this.transactionMapper.toDomain(transaction)).toList();
        ...
        public PaginatedResult<Transaction> search(List<FilterParam<?>> criteria, List<FilterParam<?>> search,
                ...
                List<Transaction> transactions = this.entityManager.createQuery(cq).setFirstResult(offset)
                                ...
                                .stream()
                                .map((transaction) -> this.transactionMapper.toDomain(transaction)).toList();
                long count = Util.<PrintTransaction>countByCriteria(entityManager, cb, search, criteria,
                                PrintTransaction.class);
                return PaginatedResult.of(transactions, pagniate.getPage(), pagniate.getSize(), count);
